...
class TestMetarParserNZ:
  ...
  def test_parse_metar_nzch(self):
    metar = 'NZCH 301900Z AUTO 04003KT 9999 -RA BKN070/// BKN090/// 10/09 ' \
            'Q1011 NOSIG'
    parser = MetarParserNZ()
    res = parser.parse_tokens(metar.split())
    assert_equals(parser.parsed_metar["is_special_report"], False)
    assert_equals(parser.parsed_metar["icao_id"], 'NZCH')
    assert_equals(parser.parsed_metar["obs_datetime"], '301900Z')
    assert_equals(parser.parsed_metar["mod_auto"], True)
    assert_equals(parser.parsed_metar["wind_dir_speed"], '04003KT')
    assert_equals(parser.parsed_metar["vis"], '9999')
    assert_equals(parser.parsed_metar["wx_phenomena"], ['-RA'])
    assert_equals(parser.parsed_metar["sky_condition"], ['BKN070///', \
                                                         'BKN090///'])
    assert_equals(parser.parsed_metar["temp"], '10')
    assert_equals(parser.parsed_metar["dewpoint"], '09')
    assert_equals(parser.parsed_metar["altimeter"], 'Q1011')
